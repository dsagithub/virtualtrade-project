(function($) {
	...
	router.add = function(route, id, callback) {
		...
		if (!isRegExp) {
			...
			// if the routes where created with an absolute url ,we have to
			// remove the absolut part anyway, since we cant change that much
			route = route.replace(location.protocol + "//", "").replace(
					location.hostname, "");
		}
		...
		// we add the event listener after the first route is added so that we
		// dont need to listen to events in vain
		if (!eventAdded) {
			bindStateEvents();
		}
	};
	...
	function bindStateEvents() {
		...
		// default value telling router that we havent replaced the url from a
		// hash. yet.
		...
		if (hasPushState) {
			...
			if (location.hash.indexOf("#!/") === 0) {
				...
				// this flag tells router that the url was converted from hash
				// to popstate
			...
			// if no events are available we use a timer to check periodically
			// for changes in the url
	...
	router.go = function(url, title) {
		if (hasPushState) {
			...
			url = url.replace(location.protocol + "//", "").replace(
					location.hostname, "");
	...
	// get the current parameters for either a specified url or the current one
	// if parameters is ommited
	...
	function getParameters(url) {
		...
		for (var i = 0, ii = routeList.length; i < ii; i++) {
			...
			// check for mathing string routes
				...
				// first check so that they have the same amount of elements at
				// least
	...
	if (!$.router) {
		...
		if (window.console && window.console.warn) {
			console
					.warn("jQuery.status already defined. Something is using the same name.");
		}
	}
